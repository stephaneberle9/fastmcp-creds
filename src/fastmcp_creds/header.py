    ...
    def get_credentials(self) -> tuple[str | None, str | None]:
        try:
            ...
            if self.username_header_name is None:
                token = headers.get(self.password_header_name.lower()) or headers.get(self.password_header_name)
                if token and is_unresolved_placeholder_value(token):
                    logger.debug(f"Ignoring unresolved placeholder in '{self.password_header_name}' header")
                    token = None
                if token:
                    logger.debug(f"Successfully extracted token '{token[:7]}***' from {self.password_header_name} header")
                    return token, token
                logger.debug(f"No valid '{self.password_header_name}' header found")
                return None, None

            username = headers.get(self.username_header_name.lower()) or headers.get(self.username_header_name)
            password = headers.get(self.password_header_name.lower()) or headers.get(self.password_header_name)

            if username and is_unresolved_placeholder_value(username):
                logger.debug(f"Ignoring unresolved placeholder in '{self.username_header_name}' header")
                username = None
            ...
            if username and password:
                logger.debug(f"Successfully extracted credentials for '{username[:7]}***' from {self.username_header_name} and {self.password_header_name} headers")
                return username, password
            logger.debug(f"No valid '{self.username_header_name}' and '{self.password_header_name}' headers found")
            return None, None
